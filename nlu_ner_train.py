...
class JointModel(paddle.nn.Layer):
    def __init__(self, ernie, num_slots, dropout=None):  #num_intents, use_history=False
...
class JointLoss(paddle.nn.Layer):
    ...
    def forward(self, slot_logits, slot_labels):
        slot_loss = self.slot_criterion(slot_logits, slot_labels)
        loss = slot_loss
        return loss
...
def nlu_model_train(
    ...
    if not os.path.exists(os.path.join(project_path, 'ckpt_ner')):
        os.makedirs(os.path.join(project_path, 'ckpt_ner'))
    save_path = os.path.join(project_path, "ckpt_ner", "best.pdparams")
    ...
    logger.info('model name for NER: ')
    ...
    if not lang:
        ...
        joint_model = JointModel_M(ernie, len(slot2id), dropout=0.1)
    ...
    for epoch in range(1, num_epoch+1):
        for batch_data in train_loader:
            ...
            loss = joint_loss(slot_logits, tag_ids)
...
def evaluate(joint_model, data_loader, slot_metric): # intent_metric
...
if __name__=='__main__':
    project_path = 'data/project/immd'
    nlu_model_train(logger, project_path)